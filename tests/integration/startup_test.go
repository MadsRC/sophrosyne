...
func TestStartup(t *testing.T) {
	...
	t.Run("Text param value is passed along to the upstream check", func(t *testing.T) {
		res, err := doAuthenticatedRequest(t, &te, "POST", []byte(`{"jsonrpc":"2.0","id":"1234","method":"Scans::PerformScan","params":{"text":"false"}}`))
		require.NoError(t, err)
		expected := []byte(`{"jsonrpc":"2.0","result":{"result":false,"checks":{"dummycheck":{"status":false,"detail":"this was false"}}},"id":"1234"}`)
		compareResponse(t, expected, res)
	})
}
